...
def classify_final_label(
    *,
    ...
    if not left_structure_ok:
        if (
            base_label in {"partial match", "weak-crown variant"}
            and structural_score >= 50.0
            and exemplar_consistency_score >= 55.0
            and pre_breakout_base_score >= 65.0
            and nearest_distance <= 0.20
        ):
            return "weak-crown variant" if base_label == "weak-crown variant" else "partial match"

        if (
            structural_score >= 40.0
            and exemplar_consistency_score >= 40.0
            and pre_breakout_base_score >= 45.0
...
async def build_automatic_market_universe(
    ...
    # Keep debug and skip reasons compact for automatic scans to avoid oversized payloads.
    selected_keys = {make_asset_key_from_id(cid) for cid in market_batch_ids}
    if not req.debug and req.compact_response:
        local_debug = {key: value for key, value in local_debug.items() if key in selected_keys}
        local_skip_reasons = {key: value for key, value in local_skip_reasons.items() if key in selected_keys}
    else:
        local_debug = {
            key: value
...
def compact_scan_result(result: ScanResult, include_notes: bool = False) -> ScanResult:
...
async def scan_pattern(req: ScanRequest) -> ScanResponse:
    ...
    try:
        ...
        if req.compact_response:
            final_results = [compact_scan_result(x, include_notes=req.include_notes) for x in final_results]
            if req.symbols or req.coingecko_ids:
                final_prefilter = [compact_scan_result(x, include_notes=req.include_notes) for x in final_prefilter]
            else:
                final_prefilter = []
        ...
        response_skip_reasons = skip_reasons
        response_debug = debug_by_symbol
        if req.compact_response and not req.debug and not (req.symbols or req.coingecko_ids):
            selected_keys = set(evaluated_assets) | set(skipped_assets)
            response_skip_reasons = {k: v for k, v in skip_reasons.items() if k in selected_keys}
            response_debug = {k: v for k, v in debug_by_symbol.items() if k in selected_keys}
        ...
        return ScanResponse(
            ...
            debug_by_symbol=response_debug if req.debug or not req.compact_response or req.symbols or req.coingecko_ids else {},
